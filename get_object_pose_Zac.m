function [mat_R_T_G, mat_R_T_M] = get_object_pose_Zac(model_name,get_robot_gripper_pose_flag)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
    ...
    z_offset = -0.18; %0.052; % Can height is 5.2cm
    ...
    %mat_R_T_M(1:3,1:3) = fing_along_x(1:3,1:3);

   % mat_R_T_M(1:3, 1:3) = round(mat_R_T_M(1:3, 1:3))

    %% 3. Adjustments to make the colision boxes accurate
    %Roll = eul2tform([-0.1 .085 -0.1]);
    %mat_R_T_M(1:3,1:3) = Roll(1:3,1:3)*mat_R_T_M(1:3,1:3);
