% this function is called by moveTo3.m
% helps calculate trajectories

function des_q = convertPoseTraj2JointTraj_new_RRT(robot,mat_traj,toolFlag)
    %-------------------------------------------------------------
    ...
    for i = 1:num_traj_points
        [des_q, ~] = ik('tool0',mat_traj(:,:,i),ikWeights,[0 pi/4 pi/2 -pi/4 0 0]); % Should we update initial guess with the latest q_traj value?
